...
.directive('json', function($compile) {
  return {
    ...
    link: function(scope, element, attributes) {
        ...
        scope.addItem = function(obj) {
            ...
            if (getType(obj) == "lObject") {
                ...
                switch(scope.valueType) {
                    ...
                    case textName: obj.push(scope.valueName ? scope.valueName : "ciao");
                                    break;
        ...
        // recursion
        var switchTemplate = function(newType){
        	console.log("new Type "+newType);
        	if(newType=="String")
        		{
        		return  '<span  class="jsonLiteral"><input type="text" ng-model="val" '
                + 'placeholder="Empty" ng-model-onblur ng-change="child[key] = possibleNumber(val)"/>'
            + '</span>'
        		}
        	else if(newType=="Boolean")
        		{
        		return '<span  class="jsonLiteral"><input type="checkbox" ng-model="val" '
                + 'placeholder="Empty" ng-model-onblur ng-change="child[key] = possibleNumber(val)"/>'
            + '</span>'
        		}
        	else if(newType=="Number")
        		{
        		return  '<span  class="jsonLiteral"><input type="number" ng-model="val" '
                + 'placeholder="Empty" ng-model-onblur ng-change="child[key] = possibleNumber(val)"/>'
            + '</span>'
        		}
        	else {

        		return '<json child="val" type="'+newType+'" default-collapsed="defaultCollapsed">devo aggiungere</json>';
        ...
        var pare="String";
        var addItemArrayTemplate =
        '<div ng-switch on="showAddKey" class="block" >'
            + '<span ng-switch-when="true">'
            	+'<span> : child {{child}} <input type="text" placeholder="Value"'
             + 'class="form-control input-sm addItemValueInput" ng-init="$parent.valueType=\''+pare+'\'"  ng-model="$parent.valueName" ui-keyup="{\'enter\':\'addItem(child)\'}"/></span> '
         // Add button
 ...
 + '</div>';
        ...
        var dataType=scope.type.split("[");
        var arrayDimension = scope.type.split("[").length;
     ...
     //
          ...
          // start template
        if (arrayDimension>2) {
  		  var newType=scope.type.substr(0,	scope.type.length-2);
          ...
          + '<div class="jsonContents" ng-hide="collapsed">'
              + '<ol class="arrayOl" ng-model="child">'
                  // repeat
                      + '<span>' + switchTemplate(newType) + '</span>'
                  // repeat end
              + '</ol>'
              + addItemArrayTemplate
          + '</div>';
            ...
            var template = '<i>Number: </i><input type="number" ng-model="val" '
                + 'placeholder="Empty" ng-model-onblur ng-change="child[key] = possibleNumber(val)"/>';


            } else if (dataType[0] == "String") {
                var template ='<i>String: </i><input type="text" ng-model="val" '
                    + 'placeholder="Empty" ng-model-onblur ng-change="child[key] = possibleNumber(val)"/>';
            } else if (dataType[0] == "Boolean") {
                        var template = '<i>Boolean: </i><input type="checkbox" ng-model="val" '
                            + 'placeholder="Empty" ng-model-onblur ng-change="child[key] = possibleNumber(val)"/> <span>{{val}}</span>';


                        } else if (dataType[0] != undefined){
            ...
            console.error("scope.type was "+ dataType);
        }
