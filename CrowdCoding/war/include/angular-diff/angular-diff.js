...
angular.module('diff', [])
		.filter(
				...
				function($sce) {
					 ...
					 * (http://ejohn.org/) Modified by Chu Alan 'sprite'
					 *
					...
					function diffString2(o, n) {
						...
						for (i = 0; i < out.o.length; i++) {
							...
							if (out.o[i].text) {
								os += '<span style="background-color: '
										+ colors[i] + '">'
										+ escape(out.o[i].text) + oSpace[i]
										+ '</span>';
							} else {
								os += '<del>' + escape(out.o[i]) + oSpace[i]
										+ '</del>';
							}
						}
						...
						for (i = 0; i < out.n.length; i++) {
							if (out.n[i].text) {
								ns += '<span style="background-color: '
										+ colors[out.n[i].row] + '">'
										+ escape(out.n[i].text) + nSpace[i]
										+ '</span>';
							} else {
								ns += '<ins>' + escape(out.n[i]) + nSpace[i]
										+ '</ins>';
							}
						}
					...
					function diff(o, n) {
						...
						for (i = n.length - 1; i > 0; i--) {
							if (n[i].text && !n[i - 1].text && n[i].row > 0
									&& !o[n[i].row - 1].text
									&& n[i - 1] === o[n[i].row - 1]) {
								n[i - 1] = {
									text : n[i - 1],
					...
					//	return $sce.trustAsHtml(diffString(input, match)).toString();
...
function randomColor() {
	return "rgb(" + (Math.random() * 100) + "%, " + (Math.random() * 100)
			+ "%, " + (Math.random() * 100) + "%)";
}
function diffString2(o, n) {
	o = o.replace(/\s+$/, '');
